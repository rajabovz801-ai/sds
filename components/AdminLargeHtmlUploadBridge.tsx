...
export function AdminLargeHtmlUploadBridge() {
  useEffect(() => {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (!target || !(body instanceof FormData)) return nativeFetch(input, init);

      const file = body.get('file');
      if (!(file instanceof File) || file.size <= DIRECT_UPLOAD_THRESHOLD) return nativeFetch(input, init);

      if (!/\.html?$/i.test(file.name)) return jsonResponse('Faqat .html yoki .htm fayl qabul qilinadi.', 400);
      if (file.size > MAX_DIRECT_HTML_BYTES) return jsonResponse('HTML fayl hajmi 50 MB dan oshmasligi kerak.', 400);
      ...
      try {
        ...
        if (!signResponse.ok) return jsonResponse(signBody.error || 'HTML upload tayyorlanmadi.', signResponse.status);

        const { filePath, token } = signBody as { filePath?: string; token?: string };
        if (!filePath || !token) return jsonResponse('HTML upload tokeni olinmadi.');
        ...
        if (uploadError) return jsonResponse(`HTML yuklanmadi: ${uploadError.message}`);
        ...
        return nativeFetch('/api/admin/tests/direct-upload', {
          ...
          body: JSON.stringify({
            ...
            testScope: field(body, 'testScope'),
            testCollection: field(body, 'testCollection'),
