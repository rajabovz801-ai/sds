...
import {ArrowRight,BookOpen,CheckCircle2,Highlighter,ChevronLeft,ChevronRight,AlertCircle} from "lucide-react";
...
export default function ReadingAnalysis({passage,review,onBack,onNext,nextAvailable,nextTitle}:Props){
 ...
 return <div className="ra-shell">
  ...
  <footer className="ra-footer"><AnimatedBackButton onClick={onBack} ariaLabel="Back to all practices"/><div><span>{passage.ordinal===1?"Finish this review, then continue.":"Both practices can be reviewed at any time."}</span>{nextAvailable&&<button className="ra-next" onClick={onNext} title={nextTitle||nextLabel}>{nextLabel}<ArrowRight size={17}/></button>}</div></footer>
  <style jsx global>{`
