...
type Q={number:number,type:"tfng"|"gap"|"mcq"|"select",text:string,options?:string[],instruction?:string,pair?:string};
...
export default function ChallengeReading(){
 ...
 const passageType=[2,6,9,13,14].includes(day)?"Passage 2":"Passage 1";
 ...
 return <main className={"cr-shell "+(!passage?"cr-overview":"")}>
   ...
   <section style={{display:tab==="passage"?"var(--cr-hide-questions)":"block"}} className="cr-pane cr-questions" ref={questionsRef} onMouseUp={showHighlightMenu} onTouchEnd={()=>setTimeout(showHighlightMenu,100)}>
    {questionGroups.map((group,gi)=><section className="cr-qgroup" key={gi}><div className="cr-qgroup-head"><h3>Questions {group[0].number}{group.length>1?"–"+group[group.length-1].number:""}</h3>{group[0].instruction&&<p>{group[0].instruction}</p>}</div>
     {group.map(q=><div className="cr-question" key={q.number} id={"question-"+q.number}><div className="cr-question-head"><b>{q.number}</b><span>{q.text}</span></div>
      {q.type==="tfng"?<div className="cr-options">{["TRUE","FALSE","NOT GIVEN"].map(opt=><label key={opt}><input type="radio" disabled={!!result} checked={answers[q.number]===opt} onChange={()=>setAnswers(a=>({...a,[q.number]:opt}))}/><span className="cr-radio"/>{opt}</label>)}</div>:q.type==="mcq"?<div className="cr-options">{(q.options||[]).map(opt=><label key={opt}><input type="radio" disabled={!!result} checked={answers[q.number]===opt[0]} onChange={()=>setAnswers(a=>{const next={...a,[q.number]:opt[0]};if(q.pair)for(const other of passage.questions){if(other.number!==q.number&&other.pair===q.pair&&next[other.number]===opt[0])delete next[other.number];}return next;})}/><span className="cr-radio"/>{opt}</label>)}</div>:q.type==="select"?<select className="cr-gap cr-select" aria-label={"Answer question "+q.number} disabled={!!result} value={answers[q.number]||""} onChange={e=>setAnswers(a=>({...a,[q.number]:e.target.value}))}><option value="">Choose your answer</option>{(q.options||[]).map(opt=>{const code=opt.split(" ")[0].replace(/[.:)]/g,"");return <option key={opt} value={code}>{opt}</option>})}</select>:<input className="cr-gap" type="text" placeholder="Your answer" autoComplete="off" spellCheck={false} disabled={!!result} value={answers[q.number]||""} onChange={e=>setAnswers(a=>({...a,[q.number]:e.target.value}))}/>}
  ...
  .cr-select{min-width:240px;max-width:calc(100% - 45px);padding:10px 12px;background:#fff;color:#182c41;font-weight:650;white-space:nowrap;text-overflow:ellipsis;cursor:pointer}
