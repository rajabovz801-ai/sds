 ...
 return <main className="cr-shell">
  ...
  {!passage?<section className="cr-picker">
   ...
   <section style={{display:tab==="passage"?"var(--cr-hide-questions)":"block"}} className="cr-pane cr-questions" ref={questionsRef} onMouseUp={()=>selectionHandler("questions")} onTouchEnd={()=>selectionHandler("questions")}>
    {passage.questions.map((q,i)=><div className="cr-question" key={q.number} id={"question-"+q.number}><div className="cr-question-head"><b>{q.number}</b><span>{q.text}</span></div>
      {q.type==="tfng"?<div className="cr-options">{["TRUE","FALSE","NOT GIVEN"].map(opt=><label key={opt}><input type="radio" disabled={!!result} checked={answers[q.number]===opt} onChange={()=>setAnswers(a=>({...a,[q.number]:opt}))}/>{opt}</label>)}</div>:q.type==="mcq"?<div className="cr-options">{(q.options||[]).map(opt=><label key={opt}><input type="radio" disabled={!!result} checked={answers[q.number]===opt[0]} onChange={()=>setAnswers(a=>({...a,[q.number]:opt[0]}))}/>{opt}</label>)}</div>:<input className="cr-gap" type="text" placeholder="Your answer" autoComplete="off" disabled={!!result} value={answers[q.number]||""} onChange={e=>setAnswers(a=>({...a,[q.number]:e.target.value}))}/>}
      {result?.answers&&<small className="cr-correct">Correct answer: {String(result.answers[i]?.correct??"")}</small>}
    </div>)}</section></div>
   <footer className="cr-footer"><button className="cr-back-btn" onClick={()=>{setPassage(null);setResult(null);setStarted("");}}>← Passages</button><span>{result?"Completed":answered+" / "+total+" answered"}</span>{result?<button className="cr-submit" onClick={()=>{setPassage(null);setResult(null);setAnswers({});}}>Score {result.score}/{result.total} · Continue <ChevronRight size={16}/></button>:<button className="cr-submit" disabled={busy} onClick={()=>submit(false)}><Send size={15}/> {busy?"Submitting…":"Submit"}</button>}</footer>
  </>}
