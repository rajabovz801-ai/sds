...
export async function GET(request: NextRequest) {
  ...
  const startBody = await start.json().catch(() => null) as { sessionId?: string } | null;
  ...
  return NextResponse.json({
    ...
    start: { status: start.status, ok: start.ok, sessionId: startBody.sessionId, resumed: (startBody as any)?.resumed },
