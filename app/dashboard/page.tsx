...
export default function Dashboard(){
 ...
 return <div className="learning-shell">
  <aside className={"learning-sidebar "+(sidebar?"open":"")}>
   <div className="learning-brand"><div className="brand-emblem"><ArkMark size={51}/></div><div><b>ARK <span>IELTS</span></b><small>60-DAY CHALLENGE</small></div><button className="mobile-close" aria-label="Close menu" onClick={()=>setSidebar(false)}><X size={18}/></button></div>
    ...
    {view==="Dashboard"||view==="60-Day Plan"?<>
     ...
     <div className="study-layout">
      ...
      <aside className="detail-panel" id="day-detail"><div className="day-details-top"><div className="detail-overline">{chosen.mock?"FULL IELTS MOCK":"DAILY TRAINING PLAN"}</div><div className="day-row"><h2>Day {String(chosen.n).padStart(2,"0")}</h2><span className={"detail-status "+(live?"ready":"locked")}>{live?"Available":"Locked"}</span></div><p>{format(chosen.date,{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p><div className="detail-subhead"><span>{chosen.mock?"4 exam sections · continuous session":"6 learning modules"}</span><span>{chosen.mock?"No breaks":"IELTS practice"}</span></div></div>
       <div className="detail-list">{display.map(({name,detail,icon:Icon,tone,time},i)=><div key={name} className="detail-item"><span className={"task-icon "+tone}><Icon size={19} strokeWidth={1.8}/></span><div className="task-copy"><strong>{name}</strong><small>{detail}</small><span><Clock3 size={11}/>{time}</span></div><span className="task-action">{live?(stats?.completed.some(x=>x.day_number===chosen.n&&x.module===name.toLowerCase())?"Done":"Pending"):"Locked"}{live?<ChevronRight size={15}/>:<LockKeyhole size={13}/>}</span></div>)}</div>
