   ...
   <main className="learning-content">
    {view==="Dashboard"||view==="60-Day Plan"?<>
     ...
     <div className="study-layout">
      ...
      <aside className="detail-panel" id="day-detail"><div className="day-details-top"><div className="detail-overline">{chosen.mock?"FULL IELTS MOCK":"DAILY TRAINING PLAN"}</div><div className="day-row"><h2>Day {String(chosen.n).padStart(2,"0")}</h2><span className="day-inline-date">{format(chosen.date,{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</span><span className={"detail-status "+(live?"ready":"locked")}>{live?"Available":"Locked"}</span></div><p>{chosen.mock?"Complete all four exam sections in one uninterrupted session.":chosen.n===14?"Reading and Vocabulary have moved to Day 16.":"Complete all six modules to finish the day’s study."}</p><div className="detail-subhead"><span>{chosen.mock?"4 exam sections · continuous session":chosen.n===14?"4 active modules · 2 moved to Day 16":"6 learning modules"}</span><span>{chosen.mock?"No breaks":"IELTS practice"}</span></div></div>
       <div className="detail-list">{display.map(({name,detail,icon:Icon,tone,time})=>{
        const moved=chosen.n===14&&(name==="Reading"||name==="Vocabulary");
        return <div key={name} className="detail-item"><span className={"task-icon "+tone}><Icon size={19} strokeWidth={1.8}/></span>
          <div className="task-copy"><strong>{name}</strong><small>{moved?"Moved to Day 16 · 16 October":detail}</small><span><Clock3 size={11}/>{moved?"Rescheduled":time}</span></div>
          <button type="button" className={"task-action "+(moved?"future":live?"can-open":"future")} disabled={!moved&&!live}
            title={moved?"View Day 16":!live?"Available on "+format(chosen.date,{day:"numeric",month:"long"}):"Open "+name}
            onClick={()=>{if(moved){setSelected(16);return}if(live)window.location.assign(name==="Reading"&&!chosen.mock?"/day/"+chosen.n+"/reading":"/day/"+chosen.n)}}>
            {moved?"Day 16":live?(stats?.completed.some(x=>x.day_number===chosen.n&&x.module===name.toLowerCase())?"Done":"Start"):"Locked"}
            {moved||live?<ChevronRight size={15}/>:<LockKeyhole size={13}/>}
          </button></div>
       })}</div>
       <div className="day-detail-footer"><div><span>DAY STATUS</span><b>{chosen.n===14?"Reading & Vocabulary: Day 16":live?"Materials pending upload":"Available on "+format(chosen.date,{day:"numeric",month:"short"})}</b></div><button disabled={!live} className="detail-btn" onClick={()=>{if(live)location.href="/day/"+chosen.n}}>{live?"View study day":"Future day locked"}<ChevronRight size={16}/></button></div></aside>
     </div>
